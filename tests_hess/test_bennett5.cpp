...
class Bennett5HessianTest : public ::testing::Test {
protected:
    void SetUp() override {
        ...
        x_values = {7.447168E0, 8.102586E0, 8.452547E0, 8.711278E0, 8.916774E0,
        ...
        y_values = {-34.834702E0, -34.393200E0, -34.152901E0, -33.979099E0, -33.845901E0,
    ...
    const std::vector<double> b_scalar = {-2.5235058043E+03,
                                          4.6736564644E+01,
                                          9.3218483193E-01};
    ...
    auto bennett5(double x, double y, const std::vector<DualType>& b) {
        return y - b[0] * pow(b[1] + x, -1.0 / b[2]);
    }
};

TEST_F(Bennett5HessianTest, DirectionalHessianAgreement) {
    constexpr double tolerance = 1e-14;

    for (size_t i = 0; i < x_values.size(); ++i) {
        auto nested_result = bennett5(x_values[i], y_values[i], b_nested);
        auto generalized_result = bennett5(x_values[i], y_values[i], b_generalized);
...
TEST_F(Bennett5HessianTest, Performance) {
    ...
    for (int i = 0; i < runs; ++i) {
        control_sum += bennett5(x_values[i%x_values.size()], y_values[i%y_values.size()], b_nested).x.x;
    }
    timer.stop();
    ...
    timer.start();
    for (int i = 0; i < runs; ++i) {
        control_sum += bennett5(x_values[i%x_values.size()], y_values[i%y_values.size()], b_generalized).x;
    }
    timer.stop();
    double generalized_time = timer.total_time();
    //std::cout << control_sum << std::endl;
    GTEST_LOG_(INFO) << "\nPerformance Results for Bennett5 (avg ns/op):\n"
