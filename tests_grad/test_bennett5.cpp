...
class Bennett5Test : public ::testing::Test {
protected:
    void SetUp() override {
        b_dual = {
                dir_hessian_type(d_scalar(-2.5235058043E+03, 1), 0),
                dir_hessian_type(d_scalar(4.6736564644E+01, 1), 1),
                dir_hessian_type(d_scalar(9.3218483193E-01, 1), 2)
        };
    ...
    dir_hessian_type
    bennett5(double x, double y, std::vector<dir_hessian_type> b) {
        return y - b[0] * pow(b[1] + x, -1.0 / b[2]);
    }

    std::vector<long double> bennett5_grad(long double x, long double y, std::vector<long double> b) {
        return {
                -pow(b[1] + x, -1.0L / b[2]),
                b[0] * std::pow(b[1] + x, -1.0L / b[2] - 1.0L) / b[2],
                -b[0] * std::log(b[1] + x) / (pow(b[1] + x, 1.0L / b[2]) * b[2] * b[2])
        };
    }
};

TEST_F(Bennett5Test, GradientComparison) {
    constexpr long double tolerance = 5e-16;

    long double total_error = 0.0L;
    for (size_t i = 0; i < x_values.size(); ++i) {
        dir_hessian_type eps = bennett5(x_values[i], y_values[i], b_dual);
        dual::getGradient(eps);
        auto grad = bennett5_grad(x_values[i], y_values[i], b_scalar);
