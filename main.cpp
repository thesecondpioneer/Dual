...
double f(double x, double y, double b1, double b2, double b3, double b4) {
    return y - b1 * pow(1.0 + exp(b2 - b3 * x), -1.0 / b4);
}
...
ceres::Jet<double, 4> autodcr(double x, double y, ceres::Jet<double, 4> b1, ceres::Jet<double, 4> b2, ceres::Jet<double, 4> b3, ceres::Jet<double, 4> b4) {
    return y - b1 * pow(1.0 + exp(b2 - b3 * x), -1.0 / b4);
}
int main() {
    ...
    ceres_time.start();
    for (double x = 1; x <= 100; x += 0.001) {
        ceres_deriv = autodcr(x, 3.0, ceres::Jet<double, 4>(1.0, 0), ceres::Jet<double, 4>(1.0, 1), ceres::Jet<double, 4>(1.0, 2), ceres::Jet<double, 4>(1.0, 3));
    }
    ceres_time.stop();
    dual_time.start();
    for (double x = 1; x <= 100; x += 0.001) {
    ...
    std::cout << std::string(33, ' ') << "Last function value  " << "  Last derivative value" << std::string(63, ' ') <<"Time" << std::endl
              ...
              << ceres_deriv.v.transpose() << "     " << ceres_time.total_time() << std::endl;
    return 0;
}