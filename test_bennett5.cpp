...
dir_hessian_type
bennett5(long double x, long double y, dir_hessian_type b[3]){
    return y - b[0] * pow(b[1] + x, -1.0L / b[2]);
}
long double *bennett5_grad(long double x, long double y, long double b[3]){
...
int main() {
    ...
    bool equiv = true;
    for(int i = 0; i < 154; i++){
        dir_hessian_type eps = bennett5(x[i], y[i], b);
        long double *grad = bennett5_grad(x[i], y[i], b_scal);
        for(int j = 0; j < 3; j++){
