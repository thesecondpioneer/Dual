...
dir_hessian_type
mgh09(double x, double y, dir_hessian_type b[4]){
    return y - b[0] * (x * x + x * b[1]) / (x * x + x * b[2] + b[3]);
}
long double *mgh09_grad(long double x, long double y, long double b[4]){
...
int main() {
    ...
    bool equiv = true;
    for(int i = 0; i < 11; i++){
        dir_hessian_type eps = mgh09(x[i], y[i], b);
        dual::getGradient(eps);
        long double *grad = mgh09_grad(x[i], y[i], b_scal);
        for(int j = 0; j < 4; j++){
